...
func TestDurableConversationEventRunIDRoundTrip(t *testing.T) {
	db, err := openDatabase(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	...
	a := &app{db: db, files: files}
	c := durableConversation{ID: "conversation-run", Events: []durableAgentEvent{{Kind: "task", Text: "[]", RunID: "run-1"}}}
	if err := a.saveConversation("account-a", &c); err != nil {
		t.Fatal(err)
	}
	items, err := a.loadConversations("account-a")
	...
	if len(items) != 1 || len(items[0].Events) != 1 || items[0].Events[0].RunID != "run-1" {
		t.Fatalf("run identity was not preserved: %#v", items)
	}
}
