...
Warden follows `gantry-core/docs/CLI_API_CLUSTER_ROADMAP.md`. Phase 1 CP8 adopts the v0.1.1 contract for setup status, alert update and user management. Its CLI names are reserved, not marked implemented; Phase 5 must wire and runtime-observe them before executable coverage is advertised.
...
```html
<link rel="stylesheet" href="@path('public/assets/site.css')">
...
`gantry-core` checkout through the top-level `go.work` generated by
`../../gantry-core/scripts/configure-local-workspace.sh`. Keep PTY,
