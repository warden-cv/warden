...
- Opening/changing an Editor workspace also retargets the shared terminal cwd to that workspace; closing the workspace returns it to the configured home start. Explorer exposes a one-click “Open as Workspace” action for its current directory, and the Open Workspace prompt defaults to Explorer’s current directory. The PTY backend reads Bash’s Linux `/proc/<pid>/stat` foreground process-group (`tpgid`) state and queues the `cd` until Bash owns its controlling terminal again, so cwd handoff does not depend on shell prompt customisation and does not inject into a running child process.
